...
## RIDISTRIBUZIONE DEL MOMENTO ##
Mo=symbols ('Mo')

## CONDIZIONI A CONTORNO TRATTO 1 ##
ccc1=Eq(v1.subs(s,0),0) #spostamento nullo in A
ccc2=Eq(dv1.subs(s,0),0) #rotazione nulla in A
ccc3=Eq(v1.subs(s,a),v2.subs(s,a)) #congruenza sullo spostamento in C
ccc4=Eq(T1.subs(s,a),3*P/2) #Taglio in C
ccc5=Eq(dv1.subs(s,a),dv2.subs(s,a)) #congruenza sulla rotazione in C
ccc6=Eq(M1.subs(s,0),Mo) #congruenza sul momento in A

## CONDIZIONI A CONTORNO TRATTO 2 ##
ccc7=Eq(dv2.subs(s,2*a),0) #rotazione nulla in B
ccc7=Eq(T2.subs(s,2*a),P/2) #Taglio in B
ccc8=Eq(M1.subs(s,a),M2.subs(s,a)) #congruenza sul momento in C
#RISOLVO IL SISTEMA
sol2=solve((ccc1,ccc2,ccc3,ccc4,ccc5,ccc6,ccc7,ccc8),(c1,c2,c3,c4,c5,c6,c7,c8))
#print(sol2.keys ())
vr1=v1.subs(c1,sol2[c1]).subs(c2,sol2[c2]).subs(c3,sol2[c3]).subs(c4,sol2[c4])
vr2=v2.subs(c5,sol2[c5]).subs(c6,sol2[c6]).subs(c7,sol2[c7]).subs(c8,sol2[c8])
Mr1=M1.subs(c1,sol2[c1]).subs(c2,sol2[c2])
Mr2=M2.subs(c5,sol2[c5]).subs(c6,sol2[c6])
Tr1=T1.subs(c1,sol2[c1])
Tr2=T2.subs(c5,sol2[c5])


## PROBLEMA NUMERICO
Mon=0.75*M1y[0]
vnr1=vr1.subs(E,En).subs(J,Jn).subs(a,an).subs(P,Pn).subs(Mo,Mon)
vnr2=vr2.subs(E,En).subs(J,Jn).subs(a,an).subs(P,Pn).subs(Mo,Mon)
Mnr1=Mr1.subs(E,En).subs(J,Jn).subs(a,an).subs(P,Pn).subs(Mo,Mon)
Mnr2=Mr2.subs(E,En).subs(J,Jn).subs(a,an).subs(P,Pn).subs(Mo,Mon)
Tnr1=Tr1.subs(E,En).subs(J,Jn).subs(a,an).subs(P,Pn).subs(Mo,Mon)
Tnr2=Tr2.subs(E,En).subs(J,Jn).subs(a,an).subs(P,Pn).subs(Mo,Mon)

print(vnr2.subs(s,2*an))

# PLOT
yr1=np.zeros(len(s1))
yr2=np.zeros(len(s2))
Mr1y=np.zeros(len(s1))
Mr2y=np.zeros(len(s2))
Tr1y=np.zeros(len(s1))
Tr2y=np.zeros(len(s2))

for i in range(0,(len(s1))):
      yr1[i]=vnr1.subs(s,s1[i])
      Mr1y[i]=Mnr1.subs(s,s1[i])
      Tr1y[i]=Tnr1.subs(s,s1[i])
for i in range(0,(len(s2))):
      yr2[i]=vnr2.subs(s,s2[i])
      Mr2y[i]=Mnr2.subs(s,s2[i])
      Tr2y[i]=Tnr2.subs(s,s2[i])

# plot deformata
pylab.subplot(212)
pylab.plot(s1,yr1*1000,'-y', linewidth=1.5)
pylab.plot(s2,yr2*1000,'-y', linewidth=1.5)
# plot momento
pylab.subplot(221)
pylab.plot(s1,Mr1y,'-k',linewidth=1.5)
pylab.plot(s2,Mr2y,'-k',linewidth=1.5)
