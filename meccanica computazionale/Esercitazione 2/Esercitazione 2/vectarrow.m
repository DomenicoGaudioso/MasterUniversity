...
%   $Revision: 1.0
  ...
  elseif max(size(p0))==2
      if max(size(p1))==2
          ...
          x1 = p1(1);
          y1 = p1(2);
          plot([x0;x1],[y0;y1],'-r','LineWidth',1);   % Draw a line between p0 and p1
          
%           p = p1-p0;
%           alpha = 0.7;  % Size of arrow head relative to the length of the vector
...
%           
%           hold on
%           plot(hu(:),hv(:),'LineWidth',1.5,'Color','r')  % Plot arrow head
%           grid on
%           xlabel('x')
%           ylabel('y')
%           hold off
