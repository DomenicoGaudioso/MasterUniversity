...
fyk=E*np.sin(np.pi/4) # tensione di snervamento [N/mm2]
t=np.arange( 0.,2*np.pi,0.0001)  # renge di tempo [s]
...
for i in range(1,(len(t))):
    ...
    f[i]=np.abs(sigmaf(e[i],p[i]))-(A*np.exp(n)) #funzione di snervamento
    ...
    if f[i] <= 0: # FASE ELASTICA
        ...
        if sigma[i]>0 :
            p[i]=(e[i]*E-(A*np.exp(n)))/E
            sigma[i]=sigmaf(e[i],p[i]) # legame costitutivo molla
            f[i]=(sigmaf(e[i],p[i]))-(A*np.exp(n)) #funzione di snervamento

        if sigma[i]<0 :
            p[i]=(e[i]*E+(A*np.exp(np.sum(P))))/E
            sigma[i]=sigmaf(e[i],p[i]) # legame costitutivo molla
            f[i]=-(sigmaf(e[i],p[i]))-(A*np.exp(n)) #funzione di snervamento
